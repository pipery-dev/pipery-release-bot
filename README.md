...
- `.github/workflows/ci.yml` using `pipery-dev/pipery-golang-ci@v1.0.5`
...
On every push to `main` or `v*` tag, the CI workflow uses `pipery-dev/pipery-argocd-cd` to publish an ArgoCD Application and values override to `pipery-dev/pipery-argocd` under `applications/pipery-release-bot/`. Set `PIPERY_ARGOCD_TOKEN` to a token that can write to that private repository.
